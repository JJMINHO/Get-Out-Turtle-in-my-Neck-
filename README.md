...
본 프로젝트는 별도의 딥러닝 모델을 직접 학습하지 않았으며, MediaPipe에서 제공하는 사전 학습 landmark detection 모델을 활용합니다.
...
3. **최초 실행 보안 승인 (macOS Gatekeeper 우회)**:
   ...
   - 이를 실행하려면 앱 아이콘을 마우스 **우클릭(또는 Control + 클릭)** 한 뒤 **[열기]**를 선택하여 실행해 주시거나, `시스템 설정 -> 개인정보 보호 및 보안 -> 일반` 탭 하단에서 **"그래도 열기(Open Anyway)"**를 클릭해 주시면 정상 실행됩니다. (최초 1회만 필요)
...
자세 및 얼굴 landmark 추출에는 직접 학습한 모델이 아니라 MediaPipe의 사전 학습 landmark detection 모델을 사용했습니다.
