...
실시간 자세와 집중 흐름을 읽어주는 macOS 학습 코치

DeskPose Coach는 웹캠 기반 컴퓨터비전으로 사용자의 앉은 자세, 화면 집중 상태, 학습 흐름을 가볍게 분석하는 데스크톱 앱입니다. 별도 센서 없이 카메라 입력만으로 posture score, focus score, daily study score를 계산하고, 그 결과를 대시보드와 메뉴바에서 바로 확인할 수 있습니다.

이 앱은 의료용 자세 진단기나 정확한 eye tracker가 아닙니다. DeskPose Coach는 term project MVP 수준의 lightweight monitoring tool이며, 학습 습관을 돌아보기 위한 시각적 보조 도구입니다.

---

## 왜 DeskPose Coach인가

책상 앞에 오래 앉아 있으면 자세는 무너지고, 집중은 조용히 흩어집니다. DeskPose Coach는 이 흐름을 숫자와 시각화로 보여줍니다.

- 지금 자세가 안정적인지
- 화면을 보고 있는지, 자주 이탈하는지
- 오늘 얼마나 공부했고 얼마나 집중했는지
- 마감 일정 대비 지금 페이스가 괜찮은지
- 하루 학습의 양과 질이 어느 정도였는지

DeskPose Coach는 단순히 "카메라 화면을 보여주는 앱"이 아니라, 컴퓨터비전 분석 결과를 하루 단위 학습 점수와 리포트로 연결합니다.
...
하루 학습을 하나의 점수로 요약합니다. 하루 기준은 05:00부터 다음날 04:59까지입니다.
...
하루의 학습 상태를 리포트 창에서 시각적으로 확인할 수 있습니다.

- Daily Study Score
- score parts: focus density, study time, vision quality
- daily metrics progress bars
- Focus x Posture quadrant
- study state timeline
...
- 시험, 프로젝트, 마감, 과제 일정 기록
- 일정이 Gemini AI coach feedback에 반영됨

### Gemini AI Coach Feedback

Gemini API를 연결하면 Feedback 카드가 더 맥락 있는 학습 코치처럼 동작합니다.
...
DeskPose Coach의 기본 화면은 healthcare dashboard 스타일로 구성되어 있습니다.
...
- time-based study state segmentation
...
Gemini feedback을 사용하려면 프로젝트 루트에 `.env` 파일을 만듭니다.
...
배포 환경에서는 `.env` 파일을 포함하지 말고, OS 환경변수나 패키징 도구의 secret 설정으로 `GEMINI_API_KEY`를 주입하세요.
...
DeskPose Coach는 다음 오픈소스 기술을 사용합니다.
...
Gemini API는 외부 AI 서비스이며 오픈소스 라이브러리가 아닙니다.
...
- Daily Report의 더 정교한 chart rendering
- 사용자별 목표 학습 시간 설정
- 캘린더 일정 알림
...
DeskPose Coach is released under the MIT License. See [LICENSE](LICENSE) for details.
